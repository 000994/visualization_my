...
function renderRegionRadarDetail(data) {
  ...
  var c2Dom = document.getElementById("detailChart2");
  if (c2Dom && roadType.length) {
    var roadColors = ["#C85D4D", "#F0B79A", "#FAE7D9", "#E3EEEF", "#AECDD7", "#619DB8"];
    var totalCount = roadType.reduce(function(sum, r) { return sum + (r.count || 0); }, 0);

    var existingInst = echarts.getInstanceByDom(c2Dom);
    if (existingInst) {
      try { existingInst.dispose(); } catch(e) {}
    }
    c2Dom.innerHTML = '';

    var wordCloudWords = roadType.map(function(r, i) {
      return {
        name: _shortLabel(r.road_label, 16),
        value: r.count,
        share: ((r.count / totalCount) * 100).toFixed(1)
      };
    });

    if (window.WordCloud) {
      c2Dom.style.cssText = "width: 100%; height: 100%; position: relative;";
      var wc = new window.WordCloud(c2Dom, {
      ...
      title.style.cssText = "position: absolute; top: 4px; left: 10px; font-size: 11px; font-weight: bold; color: #5a5a7a; z-index: 10;";
      title.textContent = "Road Type Distribution";
      c2Dom.appendChild(title);
    } else {
      var c2 = echarts.init(c2Dom);
      c2.setOption({
        title: { text: "Road Type Distribution", left: 10, top: 4, textStyle: { fontSize: 11, color: "#5a5a7a" } },
        tooltip: { trigger: "item", formatter: function(p) {
          return tooltipHTML(p.name, [
            { label: "Accidents", value: fmt(p.value) },
            { label: "Share", value: p.percent.toFixed(1) + "%" }
          ]);
        }},
        legend: { orient: "vertical", right: 8, top: 32, itemWidth: 8, itemHeight: 8, textStyle: { fontSize: 9 } },
        series: [{
          ...
          data: roadType.map(function(r, i) {
            return { name: _shortLabel(r.road_label, 15), value: r.count, itemStyle: { color: roadColors[i % roadColors.length] } };
          })
        }]
      });
      d.push(c2);
